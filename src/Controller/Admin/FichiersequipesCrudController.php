...
class FichiersequipesCrudController extends AbstractCrudController
{
  ...
  public function createIndexQueryBuilder(SearchDto $searchDto, EntityDto $entityDto, FieldCollection $fields, FilterCollection $filters): QueryBuilder
    ...
    public function bilan_depots($typefichier, $concours) : array
    {
        ...
        ->setParameters(['edition' => $edition, 'inscrite' => true])
        ->getQuery()->getResult();
        $nbrequis = count($equipes_inscrites);
        $liste=null;
        if($typefichier!=6) {
        ...
        if($typefichier==6) {
            ...
            $i=0;
            foreach ($equipes_inscrites as $equipe) {
                if(!in_array($equipe->getIdProf1(),$listeprofs )) {
                    $listeprofs[$i] = $equipe->getIdProf1();
                    $i++;
                }
            ...
            $nbrequis = count($eleves)+count($listeprofs);
            foreach ($liste_fichiers as $fichier) {
